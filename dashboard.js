...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function getActionButtons(order) {
        if (order.status === 'pending') {
            return `<button class="btn btn-outline-dark action-btn" onclick="updateStatus(${order.id}, 'progress')">Start</button>`;
        } else if (order.status === 'progress') {
            return `<button class="btn btn-primary action-btn" style="padding: 6px 12px; font-size: 0.85rem;" onclick="updateStatus(${order.id}, 'done')">Complete</button>`;
        } else {
            return `<span style="color: green; font-size: 0.9rem"><i class="fas fa-check"></i> Notified</span>`;
        }
    }
